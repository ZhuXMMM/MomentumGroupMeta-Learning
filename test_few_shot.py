...
def main(config):
    ...
    model.eval()
    print('num params: {}'.format(utils.compute_n_params(model)))
    ...
    for epoch in range(1, test_epochs + 1):
        ...
        print('test epoch {}: acc={:.2f} +- {:.2f} (%), loss={:.4f} (@{})'.format(
                ...
                aves['vl'].item(), _[-1]))
        print('Running time: %s Seconds'%(time_total))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--load', default='./save/meta_mini-imagenet-1shot_meta-baseline-resnet12/_original/max-va.pth')
